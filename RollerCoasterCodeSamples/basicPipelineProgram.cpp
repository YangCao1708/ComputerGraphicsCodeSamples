...
int BasicPipelineProgram::Init(const char * shaderBasePath, int programNum) 
{
  if (programNum == 0)
  {
  ...
  {
      if (BuildShadersFromFiles(shaderBasePath, "ground.vertexShader.glsl", "ground.fragmentShader.glsl") != 0)
      {
          cout << "Failed to build the pipeline program." << endl;
          return 1;
      }
